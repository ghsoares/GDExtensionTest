...
class KinematicCollisionSpatial2D:
	var owner: PhysicsBodySpatial2D = null
...
func _move(params: PhysicsTestMotionParameters2D, result: PhysicsTestMotionResult2D, test_only: bool = false, cancel_sliding: bool = true) -> bool:
	...
	var colliding: bool = PhysicsServer2D.body_test_motion(m_rid, params, result)
...
func test_move(from: Transform2D, motion: Vector2, collision: KinematicCollisionSpatial2D = null, safe_margin: float = 0.08, recovery_as_collision = false) -> bool:
	...
	return PhysicsServer2D.body_test_motion(m_rid, params, res)

func add_collision_exception_with(body: PhysicsBodySpatial2D) -> void:
	assert(body)
	PhysicsServer2D.body_add_collision_exception(m_rid, body.m_rid)

func remove_collision_exception_with(body: Node) -> void:
	assert(body)
	PhysicsServer2D.body_remove_collision_exception(m_rid, body.m_rid)
